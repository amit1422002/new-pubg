...
public:
   ...
   void DrawEnemyCount(Color color, Vec2 start, Vec2 end) {
    ...
    void DrawName(Color color, const char *txt, Vec2 pos, float size) {
        if (isValid()) {
            jmethodID drawtext = _env->GetMethodID(canvasView, "DrawName",
    ...
    void DrawTriangle(Color color, Vec2 center, float size, float angle) {
        if (isValid()) {
            ...
            // Equilateral Triangle, tip always outward
            ...
            float leftX = center.x + (cos((angle + 120) * M_PI / 180.0f) * halfSize);
            float leftY = center.y + (sin((angle + 120) * M_PI / 180.0f) * halfSize);
            float rightX = center.x + (cos((angle - 120) * M_PI / 180.0f) * halfSize);
            float rightY = center.y + (sin((angle - 120) * M_PI / 180.0f) * halfSize);
...
    }
    ...
    void DrawTextBold(Color color, const char *txt, Vec2 pos, float size) {
        if (isValid()) {
            jmethodID drawtext = _env->GetMethodID(canvasView, "DrawTextBold",
                                                   "(Landroid/graphics/Canvas;IIIILjava/lang/String;FFF)V");
            jstring s=_env->NewStringUTF(txt);
            _env->CallVoidMethod(_cvsView, drawtext, _cvs, (int) color.a, (int) color.r,
                                 (int) color.g, (int) color.b,
