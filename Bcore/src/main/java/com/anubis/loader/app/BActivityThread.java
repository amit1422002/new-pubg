...
public class BActivityThread extends IBActivityThread.Stub {
    ...
    public synchronized void handleBindApplication(String packageName, String processName) {
        if (isInit())
            return;
        if (DfmStealthTier.anyBindTier(packageName)) {
    ...
    private synchronized void handleBindApplicationInner(String packageName, String processName) {
        try {
            BgmiFileLogger.setProcessLabel(processName != null ? processName : packageName);
            if (GamePackages.isBgmi(packageName)) {
                BgmiFileLogger.i("BActivityThread", "bind BGMI pkg=" + packageName + " proc=" + processName);
                // anogs/ue4 in-process patches disabled
            }
            CrashHandler.create();
        } catch (Throwable ignored) {
        }

        final boolean tierPm = DfmStealthTier.javaPmSpoof(packageName);
        final boolean espGuest = GamePackages.isBgmi(packageName) || GamePackages.isDeltaForce(packageName);
        final boolean useHostInstrumentation = tierPm || espGuest;
        ...
        HookManager.get().checkEnv(IActivityTaskManagerProxy.class);
        ...
        activityThreadAppBindData._set_instrumentationName(new ComponentName(
                useHostInstrumentation ? AnubisCore.getHostPkg() : bindData.appInfo.packageName,
                useHostInstrumentation ? AppInstrumentation.class.getName() : Instrumentation.class.getName()));
...
/*    public void loadXposed(Context context) {
    ...
    private void onBeforeCreateApplication(String packageName, String processName, Context context) {
        ensureGuestSkinCallbacksRegistered(packageName);
    ...
    private static void scheduleDeltaEspReaderIfNeeded(String packageName, int userId) {
        ...
        try {
            ...
            Slog.w(TAG, "esp reader schedule failed: " + t.getMessage());
        }
    }

    private static void ensureGuestSkinCallbacksRegistered(String packageName) {
        if (!GamePackages.isBgmi(packageName) && !GamePackages.isDeltaForce(packageName)) {
            return;
        }
        try {
            Class.forName("com.anubis.skin.EspCallbackRegistrar")
                    .getMethod("ensureRegistered")
                    .invoke(null);
        } catch (Throwable ignored) {
        }
    }

    private static void scheduleAnogsBypassIfNeeded(String packageName, String processName) {
        if (!GamePackages.isBgmi(packageName)) {
            return;
        }
        // libanogs hooks + ELF destroy disabled
        android.util.Log.i("BgmiBypass",
                "anogs hooks/ELF OFF — skipped proc=" + processName);
    }

    private static void scheduleGuestLoginHookIfNeeded(String packageName, int userId) {
        // guest login / memfd hooks disabled
    }
