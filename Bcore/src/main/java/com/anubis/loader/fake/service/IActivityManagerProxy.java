...
public class IActivityManagerProxy extends ClassInvocationStub {
    ...
    /** When true, guest binds to Google Play Services are blocked so the game runs without GMS
     *  instead of crashing with "Unknown calling package name" SecurityException from host GMS. */
    public static final boolean GMS_DISABLED = true;

    private static boolean isGmsBind(Intent intent) {
        if (intent == null) {
            return false;
        }
        ...
        if (GmsCore.GMS_PKG.equals(pkg) || GmsCore.GSF_PKG.equals(pkg)) {
            return true;
        }
    ...
    public static Object BindServiceCommon(Object who, Method method, Object[] args,int callingPackageIndex) throws Throwable {
        try {
            ...
            if (GMS_DISABLED && isGmsBind(intent)) {
                Slog.w(TAG, "GMS bind blocked (GMS disabled): " + intent);
                return 0;
            }
