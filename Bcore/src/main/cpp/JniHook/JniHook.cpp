...
static void registerNative(JNIEnv *env, jclass clazz) {
    if (clazz == nullptr) {
        ALOGE("registerNative: jni hook class is null");
        return;
    }
    JNINativeMethod gMethods[] = {
...
void JniHook::InitJniHook(JNIEnv *env, int api_level, jclass jni_hook_class,
                          jclass method_utils_class) {
    if (jni_hook_class == nullptr) {
        ALOGE("init jni hook error. JniHook class is null.");
        return;
    }
    HookEnv.api_level = api_level;
    registerNative(env, jni_hook_class);

    jclass clazz = jni_hook_class;
...
//        ALOGD("art_method_size search:0x%x",value);
    ...
    if (method_utils_class == nullptr) {
        ALOGE("init jni hook error. MethodUtils class is null.");
        return;
    }
    HookEnv.method_utils_class = (jclass) env->NewGlobalRef(method_utils_class);
