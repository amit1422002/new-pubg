...
namespace TouchInput {
    ...
    int createUInput(int ifd) {
        ...
        // Grab optional — on newer Android a failed grab still allows uinput injection.
    ...
    static int tryOpenTouchDevice(const char *devname, bool requireDirect) {
        ...
        if (requireDirect && !HasSpecificProp(fd, INPUT_PROP_DIRECT)) {
            close(fd);
            return -1;
        }
        return fd;
    ...
    int getTouchDevice() {
        ...
        while ((entry = readdir(dir))) {
            ...
            fd = tryOpenTouchDevice(devname, true);
            if (fd >= 0) {
                break;
            }
        }
        ...
        if (fd < 0) {
            ...
            while ((entry = readdir(dir))) {
                ...
                char devname[PATH_MAX];
                snprintf(devname, sizeof(devname), "%s/%s", input_path, entry->d_name);
                fd = tryOpenTouchDevice(devname, false);
                if (fd >= 0) {
                    break;
                }
            }
    ...
    void touchInputStart(int width, int height) {
        if (width <= 0 || height <= 0 || touchStart) {
            return;
        }
        ...
        if (maxContacts <= FAKE_CONTACT) {
            maxContacts = FAKE_CONTACT + 1;
        }
        contacts = (struct TouchContact *) malloc((size_t) maxContacts * sizeof(struct TouchContact));
        for (int i = 0; i < maxContacts; i++) {
            contacts[i].posX = -1;
            contacts[i].posY = -1;
    ...
    void ensureTouchInput(int width, int height) {
        if (!touchStart) {
            touchInputStart(width, height);
        }
    }
    ...
    void Test() {
        ...
        touchInputStart(1080, 2400);//Start TouchInput
